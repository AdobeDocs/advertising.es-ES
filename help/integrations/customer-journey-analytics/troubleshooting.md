...
## Lista de todos los problemas potenciales

| Problema | Más información |
| ------- | ---------------- |
| No hay llamadas alloy() visibles en la ficha [!DNL Network] de la herramienta de inspección de código del explorador. | Consulte la sección &quot;[Problemas de instalación y configuración](#issues-installation-setup)&quot; > &quot;[La extensión de WebSDK no se inicializa](#websdk-extension-doesn't-initialize)&quot; |
...
| No se realizan solicitudes de interacción o recopilación a edge.adobedc.net. | Consulte &quot;[Problemas de instalación y configuración](#issues-installation-setup)&quot; > &quot;[La extensión de WebSDK no se inicializa](#websdk-extension-doesn't-initialize)&quot; |
| Las solicitudes llegan a Adobe Experience Platform Edge Network pero devuelven errores 400 o 500. | Consulte la sección &quot;[Problemas de instalación y configuración](#issues-installation-setup)&quot; > &quot;[Flujo de datos no configurado o mal configurado](#datastream-not-configured-or-misconfigured)&quot; |
...
| No se registran conversiones de visualizaciones o clics para la página web. | Consulte la sección &quot;[Problemas de configuración de la extensión de Advertising](#advertising-extension-setup-issues)&quot; |
...
| El depurador muestra que no se cumplen las condiciones de las reglas. | Consulte la sección &quot;[Las reglas o los eventos no se activan](#rules-or-events-don't-fire)&quot; |
| La acción [!UICONTROL Send Event] nunca se ejecuta. | Consulte la sección &quot;[Las reglas o los eventos no se activan](#rules-or-events-don't-fire)&quot; |
...
* No hay llamadas alloy() visibles en la ficha [!DNL Network] de la herramienta de inspección de código del explorador.
* Error de consola: no se ha definido la aleación.
* No se realizan solicitudes de interacción o recopilación a edge.adobedc.net.

#### Posibles causas y verificación/resolución

| Causa | Fix |
| ----- | --- |
| Biblioteca no publicada o en estado de borrador | Vaya a [Flujo de publicación](https://experienceleague.adobe.com/es/docs/experience-platform/tags/publish/publishing-flow) y asegúrese de que la biblioteca que contiene la extensión WebSDK está en el estado aprobado/publicado. |
| Falta el código incrustado o el entorno es incorrecto | Compruebe que el [[!DNL Tags] código incrustado](https://experienceleague.adobe.com/es/docs/experience-platform/tags/publish/environments/environments) en la página web hace referencia al entorno correcto (Dev/Stage/Prod). Busque el entorno en la etiqueta `<head>` para la etiqueta de script `//assets.adobedtm.com/...`. |
| Conflicto de carga asíncrona frente a sincrónica | Asegúrese de que solo hay un [[!DNL Tags] código incrustado](https://experienceleague.adobe.com/es/docs/experience-platform/tags/publish/environments/environments) por página web. Los códigos incrustados duplicados causan condiciones de carrera. |
| Bloqueo de la política de seguridad de contenido (CSP) | Agregue `edge.adobedc.net` `and assets.adobedtm.com` a sus [directivas CSP `connect-src` y `script-src`](https://experienceleague.adobe.com/es/docs/experience-platform/collection/use-cases/configuring-a-csp). |

### Flujo de datos no configurado o mal configurado {#datastream-not-configured-or-misconfigured}

#### Problemas:

* Las solicitudes llegan a Adobe Experience Platform Edge Network pero devuelven errores 400 o 500.
* No aparecen datos en los informes de Adobe Analytics o Adobe Advertising.
...
| Discordancia de zona protegida | Asegúrese de que la secuencia de datos pertenezca a la misma [zona protegida de Adobe Experience Platform](https://experienceleague.adobe.com/es/docs/experience-platform/sandbox/home) que el esquema y el conjunto de datos. Un error común es crear una secuencia de datos en la zona protegida de producción, pero señalar esquemas a la zona protegida de desarrollo. |
...
| Causa | Fix |
| ----- | --- |
| Las cookies de terceros están bloqueadas. | Migre a la recopilación de datos CNAME de origen al [configurar un ID de cookie de origen en la configuración de Edge Network de la secuencia de datos](https://experienceleague.adobe.com/es/docs/experience-platform/datastreams/configure). |
...
* La regla se guardará y se incluirá en la [compilación de biblioteca](https://experienceleague.adobe.com/es/docs/experience-platform/tags/publish/builds) activa.
...
* Las condiciones de la regla no son demasiado restrictivas. Realice pruebas [eliminando condiciones](https://experienceleague.adobe.com/es/docs/experience-platform/tags/ui/rules) temporalmente para aislar el problema.
* El orden de las reglas es correcto. Si varias reglas comparten el mismo evento, compruebe la [ordenación de reglas](https://experienceleague.adobe.com/es/docs/experience-platform/tags/ui/rules).
...
| Causa | Fix |
| ----- | --- |
| Los cambios no se han añadido a una biblioteca. | En el flujo de trabajo de publicación, confirme que los cambios se han agregado a una [biblioteca](https://experienceleague.adobe.com/es/docs/experience-platform/tags/publish/libraries) en el entorno de desarrollo. Si [realiza cambios en la biblioteca](https://experienceleague.adobe.com/es/docs/experience-platform/tags/publish/libraries#manage-library-changes), agregue los recursos y, a continuación, guarde y genere la biblioteca. |
...
| El código incrustado es para el entorno incorrecto. | Confirme que el código incrustado en la página es para el entorno correcto. Por ejemplo, si está probando el comportamiento de producción, confirme que el código incrustado es el código incrustado de producción. |
| La compilación de la biblioteca falló silenciosamente. | En el flujo de trabajo de publicación, compruebe si la biblioteca muestra un estado [!UICONTROL Build Failed]. Abra la biblioteca y revise el registro de generación; las causas comunes de los errores de generación son configuraciones de regla no válidas o conflictos de versión de extensión. |
...
| Causa | Fix |
| ----- | --- |
| Falta el grupo de campos [!UICONTROL Advertising] en el esquema. | <ol><li>Vaya a Adobe Experience Platform > [!UICONTROL Data Management] > [!UICONTROL Schemas].</li><li>Abra el esquema utilizado por el conjunto de datos.</li><li>En el panel [!UICONTROL Field Groups], confirme que aparece **Adobe Advertising Cloud ExperienceEvent Full Extension**.</li><li>Si falta, selecciona **Agregar**, busca **Adobe Advertising Cloud**, selecciona **Extensión completa de Adobe Advertising Cloud ExperienceEvent** y, a continuación, guarda la configuración.</li></ol>Volver a publicar su biblioteca [!DNL Tags] no es necesario solo para los cambios de esquema, pero debe volver a asignar el elemento de datos XDM en [!DNL Tags] si se agregaron nuevos campos. |
| Faltan los campos de Adobe Advertising requeridos en el esquema. | Asegúrese de que los campos de Adobe Advertising requeridos estén presentes en el [esquema](https://experienceleague.adobe.com/es/docs/platform-learn/implement-web-sdk/initial-configuration/configure-schemas) en `_experience.adcloud.conversionDetails`. Consulte &quot;[Referencia: campos de esquema obligatorios](#required-schema-fields)&quot;.<br><br>Si falta alguno de los campos, confirme que el grupo de campos **Adobe Advertising Cloud ExperienceEvent Full Extension** se guardó en el esquema y, a continuación, actualice el editor de esquemas. |
| La dirección URL de la página de aterrizaje no incluye los parámetros de consulta necesarios. | Asegúrese de que la dirección URL de la página de aterrizaje incluye los parámetros de consulta necesarios. Al hacer clic en un anuncio, la dirección URL de la página de aterrizaje debe contener tanto los parámetros de consulta `s_kwcid` como `ef_id`, como `https://www.example.com/landing-page?s_kwcid=AL!12345!3!abc123&ef_id=abc123xyz:G:s`. Consulte &quot;[Referencia: faltan parámetros de consulta](#missing-query-parameters)&quot; para ver las causas probables. |
| Faltan algunos parámetros en la carga útil XDM o están vacíos. | Para validar la carga útil XDM saliente, abra la pestaña Adobe Experience Platform Debugger o [!DNL Network] de la herramienta de inspección de código del explorador, filtre `edge.adobedc.net` e inspeccione el cuerpo de la solicitud de interacción (consulte la carga útil de ejemplo que se muestra a continuación).<br><br>Si `trackingCode` o `trackingIdentity` están vacíos o faltan, a) el parámetro de consulta no estaba presente en la página cuando se activó la regla (compruebe la URL y el tiempo de evento de la regla) o b) el grupo de campos no está en el esquema (vuelva a visitar la primera fila de arriba). |

##### Referencia: campos de esquema obligatorios {#required-schema-fields}
...
##### Referencia: faltan parámetros de consulta {#missing-query-parameters}
...
| `ef_id` | La dirección URL de la página de aterrizaje no utiliza un redireccionamiento seguido por Adobe Advertising o la adición de EF ID no está habilitada en la configuración de la campaña. |

**Carga para clics de ejemplo**
...
## Herramientas útiles de validación y depuración
...
### Pestaña [!DNL Network] de la herramienta de inspección de código del explorador

Utilice la ficha [!DNL Network] de la herramienta de inspección de código del explorador (generalmente denominada &quot;[!DNL Inspect]&quot;) para hacer lo siguiente:
...
## Lista de comprobación de referencia rápida antes de solicitar asistencia
...
* Adobe Experience Platform Debugger confirma que los eventos llegan a Edge Network.
...
* Adobe Experience Platform Debugger confirma que `conversionDetails` se ha rellenado en la carga útil saliente.
