---
title: Código JavaScript de  [!DNL Analytics for Advertising]
description: Código JavaScript de  [!DNL Analytics for Advertising]
...
# Código JavaScript de [!DNL Analytics for Advertising]
...
En la primera vista de página de una visita al sitio, el código JavaScript de Adobe Advertising comprueba si el visitante ha visto o hecho clic anteriormente en un anuncio. Si el usuario ha entrado anteriormente en el sitio mediante un clic o no ha visto un anuncio, se ignora al visitante. Si el visitante ha visto un anuncio y no ha entrado al sitio haciendo clic durante la ventana retrospectiva de [clic](/help/integrations/analytics/prerequisites.md#lookback-a4adc) establecida en Adobe Advertising, el código JavaScript de Adobe Advertising a) usa el [servicio de Experience Cloud ID](https://experienceleague.adobe.com/docs/id-service/using/home.html?lang=es) para generar un identificador suplementario (`SDID`) o b) usa el método de Adobe Experience Platform [!DNL Web SDK] `generateRandomID` para generar un `[!DNL StitchID]`. Cualquier ID se utiliza para unir datos de Adobe Advertising a la visita de Adobe Analytics del visitante. A continuación, Adobe Analytics consulta a Adobe Advertising el AMO ID y el EF ID asociados con la exposición del anuncio. A continuación, el ID de AMO y los ID de EF se rellenan en sus respectivos [!DNL eVars]. Estos valores persisten durante un periodo designado (de forma predeterminada, 60 días).

[!DNL Analytics] envía métricas de tráfico del sitio (como vistas de página, visitas y tiempo empleado) y cualquier evento estándar o personalizado de [!DNL Analytics] a Adobe Advertising cada hora, usando el ID de EF como clave. Estas métricas de [!DNL Analytics] se ejecutan a través del sistema de atribución de Adobe Advertising para conectar las conversiones al historial de clics y exposiciones.
...
La biblioteca de JavaScript consta de dos líneas que permiten que [!DNL Analytics] y Adobe Advertising se comuniquen entre sí. Si la integración de [!DNL Analytics for Advertising] se completó durante la implementación de Adobe Advertising, ya debería haber recibido este código con instrucciones sobre cómo implementarlo.
...
#### Implementaciones que utilizan el código [!DNL Web SDK] de Experience Platform `alloy.js`
...
La función JavaScript [!DNL Analytics for Advertising] debe ir después del servicio Experience Cloud ID, pero antes que el código de App Measurement de Analytics. Esto garantiza que el identificador suplementario (`SDID`) o `[!DNL StitchID]` se incluya en la llamada de Analytics.

![Colocación de código](/help/integrations/assets/a4adc-code-placement.png)

### Validación de la implementación de código

Puede realizar la validación utilizando cualquier tipo de herramienta de detector de paquetes (como [!DNL Charles], [!DNL Fiddler] o [!DNL Chrome Developer Tools]) comparando los valores de los cuatro ID entre la solicitud que va a Adobe Advertising y la solicitud que va a [!DNL Analytics], como se describe a continuación.
...
1. (Implementaciones que usan el código `visitorAPI.js` del servicio de identidad de Experience Cloud) Filtre por `/b/ss` para ver la visita de Analytics.

   ![Filtrado en `/b/ss`](/help/integrations/assets/a4adc-code-validation-filter-bss.png)

1. (Implementaciones que usan el filtro de Experience Platform [!DNL Web SDK] `alloy.js`code) en `/interact` para comprobar que la carga de la solicitud en Edge Network contiene `advertisingStitchID`.
...
1. Abra [[!DNL Adobe Experience Cloud Debugger]](https://experienceleague.adobe.com/docs/debugger/using-v2/summary.html?lang=es) en su página de inicio.
...
1. En la fila [!UICONTROL Request - Parameters], audite las señales generadas, de forma similar al Paso 3 de &quot;[Cómo confirmar el código con [!DNL Chrome Developer Tools]](#validate-js-chrome)&quot;.
   ...
   * (Implementaciones que utilizan el código [!DNL Web SDK]Experience Platform `alloy.js`) Asegúrese de que el valor del parámetro `advertisingStitchID` coincida con el `Sdid` enviado a Experience Platform Edge Network.
