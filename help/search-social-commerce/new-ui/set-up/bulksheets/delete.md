---
title: (Nueva IU) Eliminar hojas de edición masiva y archivos de error cargados
description: Obtenga información sobre cómo eliminar los archivos de hojas de edición masiva y los archivos de error en la nueva IU de Search, Social y Commerce.
feature: Search Bulksheets
product_v2:
  - id: a829a185-511f-4bf8-8dcf-9e684f8011cf
feature_v2:
  - id: aed5e38a-3e62-42fa-8d16-cd080729b2a0
subfeature_v2:
  - id: e58024d1-d6da-420c-80af-6be211808316
  - id: f3d33161-c519-436e-bbbd-730ba428736b
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
source-git-commit: 739034010787c2016720bef37fb75dc8efbae58b
workflow-type: tm+mt
source-wordcount: 147
ht-degree: 0%

---

# (Nueva IU) Eliminar hojas de edición masiva y archivos de error cargados

Puede eliminar manualmente archivos de hojas de edición masiva, archivos de error de validación de páginas de aterrizaje y otros archivos de error. Los archivos se eliminan automáticamente 30 días después de cargarse o generarse.

>[!TIP]
>
>Si ha publicado una hoja de edición masiva pero ha recibido errores, no elimine el archivo de errores hasta que se hayan resuelto todos los errores.

1. En el menú principal, haga clic en **[!UICONTROL Setup]** \> **[!UICONTROL Bulksheets]**.

1. Seleccione la casilla de verificación situada junto a cada archivo que desee eliminar.

1. En la barra de herramientas de acciones masivas, haga clic en **[!UICONTROL Delete]**.

1. En el mensaje de confirmación, haga clic en **[!UICONTROL Confirm]** para confirmar la eliminación.

   Si se ha programado la publicación de cualquiera de las hojas de edición masiva seleccionadas, recibirá una notificación antes de confirmarlo. Al eliminar una hoja de edición masiva programada, se cancela la publicación pendiente.

>[!MORELIKETHIS]
>
>* [(Nueva IU) Acerca de la administración de datos de campaña mediante hojas de edición por lotes](about.md)
>* [(Nueva interfaz de usuario) Detener un trabajo de hoja de edición masiva en curso](stop-job.md)
